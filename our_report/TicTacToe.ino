...
// Χρησιμοποιήθηκε το έτοιμο πρόγραμμα της βιβλιοθήκες TouchScreen_Calibr_native
// ώστε να καθοριστούν τα όρια της οθόνης
const int XP=6,XM=A2,YP=A1,YM=7; //240x320 ID=0x9341
const int TS_LEFT=182,TS_RT=917,TS_TOP=939,TS_BOT=187;
...
int iSetMark[] = {0,0,0,0,0,0,0,0,0}; //αρχικοποίηση πίνακα με τις κινήσιες "0": Not set, "1": CPU, "2": Human
int iWinner = 0; //"0": Draw, "1": CPU, "2": Human
int iMoves = 1;
int iEnableButtons = 1;
bool bPressed = false; // λογική μεταβλητή ελέγχου αν πατήθηκε σημείο της οθόνης

// επιστρέφει true/false ανάλογα με το αν πατήθηκε ένα σημείο της οθόνης
// ενημεώνει τις καθολικές μεταβλητές συντεταγμένων X και y 
bool Touch_getXY(void)
{
    TSPoint p = ts.getPoint(); // ποιό σημείο της οθόνης πατήθηκε
    pinMode(YP, OUTPUT);      
    pinMode(XM, OUTPUT);
    ...
    if (pressed)
    { // οι τιμές μεταφέρθηκαν από το πρόγραμμα calibration της οθόνης
      
      pixel_x = map(p.y, 939, 187, 0, 320);
      pixel_y = map(p.x, 917, 182, 0, 240);
...
void setup() {
  ...
  Serial.println("Tic Tac Toe by Pheadra's team");
  ...
  // Σχεδιασμός ενός κόκκινου ορθογωνίου
  drawFrame(10,RED);
  // καθορισμός χρωμάτων 
  tft.setCursor(30,40);
  ...
  tft.setTextSize(2);
  tft.print("University of Aegean");
  //
  tft.setTextColor(WHITE);
  tft.setCursor(30,130);
  tft.print("CodeBenders");
  // αρχικοποίηση button
